...
export default function Products() {
  ...
  useEffect(() => {
    const getAllProducts = async () => {
      try {
        setLoading(true);
        const res = await axios.get("http://localhost:8080/api/v1/product/getallproducts");
  ...
  useEffect(() => {
    ...
    if (filters.search) {
      result = result.filter((item) =>
        (item.productName || item.name || "").toLowerCase().includes(filters.search.toLowerCase())
      );
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-pink-100 to-purple-200 pt-24 pb-10 px-4">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-7">
        ...
        <div className="w-full md:w-1/4">
          {/* 🌟 Sidebar se filter ka data `setFilters` me aa jayega */}
        ...
        <div className="flex flex-col flex-1">
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-[50vh]">
              ...
              <p className="text-lg font-medium text-gray-700">Loading amazing products...</p>
            </div>
